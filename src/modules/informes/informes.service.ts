...
export class InformesService {
  async generarPdf(informe: any): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      // Añadir título
      pdf.font(titleFont).fontSize(titleFontSize).fillColor(lineColor).text(`Informe de ${informe.tipo}`, { align: 'center' });
      pdf.moveDown();
      ...
      const drawHeaders = () => {
        ...
        columns.forEach((column) => {
          pdf.fillColor(headerBgColor).rect(xPosition, yPosition, column.width, headerHeight).fill();
          pdf.fillColor(lineColor).font(titleFont).fontSize(headerFontSize);
          pdf.text(column.header, xPosition, yPosition + (headerHeight / 2) - (headerFontSize / 2), {
            width: column.width,
            align: 'center',
          });
      ...
      const maxRowsPerPage = Math.floor((pdf.page.height - yPosition - 50) / 15);
      ...
      datos.forEach((dato) => {
        ...
        columns.forEach((column) => {
          ...
          switch (column.header) {
            case 'Nombre': value = dato.name; break;
            case 'Cantidad': value = dato.quantity.toString(); break;
            case 'Unidad': value = dato.unids; break;
            case 'Cap. Almac.': value = dato.maxCapacity.toString(); break;
            case 'Precio Compra': value = dato.inPrice.toString(); break;
            case 'Moneda': value = dato.bange; break;
            case 'Precio Venta': value = dato.outPrice.toString(); break;
            case 'Stock Mín.': value = dato.minStock.toString(); break;
          }

          // Centrar el texto en la celda horizontal y verticalmente
          pdf.text(value, xPosition, yPosition + (rowHeight / 2) - (textFontSize / 2), {
            width: column.width,
            align: 'center',
          });
      ...
      pdf.font(textFont).fontSize(8).fillColor(lineColor).text(
